...
public class CommonActivity extends AppCompatActivity {
        ...
        viewModel.obtainPermission().observe(this, new Observer<PermissionModel>() {
            ...
            public void onChanged(PermissionModel model) {
                ...
                if (!model.isExecuted()) {
                    ...
                    dispatchRequestPermission(model);
                }
            }
        });
    ...
    protected void dispatchRequestPermission(PermissionModel model) {
        PermissionHelper.requestPermission(peekViewModel(RequestViewModel.class), 100, model.permissions());
    }

    protected void dispatchRequestResult(ActivityResultModel model) {
    ...
    public boolean shouldShowRequestPermissionRationale(PermissionModel model, String permission) {
    ...
    public void show(Class<? extends DialogFragment> aClass, Class<? extends CommonViewModel> viewModelClass, Object value) {
    ...
    private void attachSupportViewModel(final CommonViewModel viewModel) {
    ...
    protected void dispatchDialogEvent(DialogModel model) {
        ...
        if (dialog instanceof CommonDialog) {
            ((CommonDialog) dialog).setCurrentDialogModel(model);
        } else {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                dialog.create();
            }
